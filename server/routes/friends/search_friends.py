...
friend=Blueprint("search_friends", __name__)

#route to search within the logged-in user's friends list
@friend.route("/search-friends", methods=["GET"])
def search_friends():
    ...
    try:
        ...
        # Only matches rows where the friendship is active and name/username matches the search term
        cursor.execute(
            """
                SELECT 
        ...
        print(f"Successfully retrieved friend search results    source:{__name__}") #log message
        return jsonify({"friends": friends_list}), 200 #frontend response
    except Exception as e:
        print(f"error: {str(e)}    source:{__name__}") #log message
        return jsonify({"message": "Server error searching friends"}), 500 #frontend response
    finally:
        if conn:
            conn.close()